...
class CeVIOcs:
    ...
    def __init__(self):
        #CeVIO起動API
        CeVIOcs.service = win32com.client.Dispatch("CeVIO.Talk.RemoteService.ServiceControl")
        service_status = CeVIOcs.service.StartHost(False)
        if service_status < 0:
            raise StartupError(service_status)

        #API接続
        self.__talker = win32com.client.Dispatch("CeVIO.Talk.RemoteService.Talker")

        #話し手取得
        if not CeVIOcs.__talker_name:
            self.get_talker()

        #初期設定
        self.__talker.Cast = CeVIOcs.__talker_name[0]
    ...
    def change_emotion(self, value:dict, mode:bool = True):
        ...
        #選択された感情値を設定
        for i in temp:
            if i in  CeVIOcs.__emotion[self.__talker.Cast]:
    ...
    def set_emotion(self, value:str):
        ...
        """
        #選択された感情値を100に
        if value in  CeVIOcs.__emotion[self.__talker.Cast]:
    ...
    def set_talker(self, name:str):
        ...
        話し手の変更
        """
        if name in CeVIOcs.__talker_name:
            self.__talker.Cast = name
            self.__talker.Components.ByName(CeVIOcs.__emotion[name][0]).name = 100
        else:
    ...
    def get_talker(self) -> None:
        ...
        #話し手一覧を取得
        CeVIOcs.__talker_name = [self.__talker.AvailableCasts.At(i) for i in range(self.__talker.AvailableCasts.Length)]
        ...
        #話し手を変更し全員の感情値を取得
        for i in CeVIOcs.__talker_name:
            self.__talker.Cast = i
            CeVIOcs.__emotion[i] = [self.__talker.Components.At(i).Name for i in range(self.__talker.Components.Length)]
    ...
    def get_emotion(self) -> dict:
        ...
        {話し手の名前(str) : 感情値(int)}
        """
        return CeVIOcs.__emotion

    def get_select_emotion(self,name:str) -> list:
        """
        選択した話し手の感情値を取得
        """
        return CeVIOcs.__emotion[name]
